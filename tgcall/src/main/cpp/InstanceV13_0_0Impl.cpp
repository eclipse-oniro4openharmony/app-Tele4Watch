...
public:
    ...
    // Pulls send-side stats off the inner cricket::VoiceChannel (audio level bars + send
...
public:
    ...
    // Pulls receive-side stats off the inner cricket::VoiceChannel — mirror of
    // OutgoingAudioChannel_13_0_0::getSendStats. Must be invoked on the worker thread.
    bool getReceiveStats(cricket::VoiceMediaReceiveInfo *info) const {
        if (!_audioChannel) {
            return false;
        }
        return _audioChannel->receive_channel()->GetStats(info, /*reset_legacy=*/false);
    }
...
private:
    ...
    void processSignalingMessage(const rtc::CopyOnWriteBuffer &data);
...
void InstanceV13_0_0ImplInternal::createSignalingConnection() {
    ...
    if (useSctp) {
        _signalingConnection = std::make_shared<SignalingSctpConnection>(_threads, onIncoming, emitData);
    } else {
...
void InstanceV13_0_0ImplInternal::sendSignalingMessage(const signaling::Message &message) {
    ...
    const auto encrypted = _signalingEncryptedConnection->encryptRawPacket(
        rtc::CopyOnWriteBuffer(packetData.data(), packetData.size()));
    if (!encrypted) {
...
void InstanceV13_0_0ImplInternal::processSignalingMessage(const rtc::CopyOnWriteBuffer &data) {
    ...
    if (const auto initialSetup = absl::get_if<signaling::InitialSetupMessage>(messageData)) {
        ...
        _networking->perform([remoteIceParameters, fingerprintHash, fingerprintValue, sslSetup](tgcalls::InstanceNetworking *networking) {
            ...
            if (!fingerprintHash.empty()) {
                fingerprint = rtc::SSLFingerprint::CreateUniqueFromRfc4572(fingerprintHash, fingerprintValue);
            }
        ...
        for (const auto &candidate : candidatesList->iceCandidates) {
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, tgcall::kTgCallLogDomain, tgcall::kTgCallLogTag,
                         ...
                         parseCandidate.candidate().type().c_str(),
...
void InstanceV13_0_0ImplInternal::emitLocalCandidate(const cricket::Candidate &candidate) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, tgcall::kTgCallLogDomain, tgcall::kTgCallLogTag,
                 ...
                 patchedCandidate.protocol().c_str(), patchedCandidate.type().c_str());
...
void InstanceV13_0_0ImplInternal::setUpMediaPipeline() {
    ...
    // Video factories intentionally nullptr — audio-only.
...
void InstanceV13_0_0ImplInternal::createNegotiatedChannels() {
    ...
    for (const auto &content : coordinatedState->incomingContents) {
        ...
        if (_incomingAudioChannel && _incomingAudioChannel->ssrc() != content.ssrc) {
            // Renegotiation: destroy on the worker thread per cricket::ChannelManager docs
...
void InstanceV13_0_0Impl::setIncomingVideoOutput(std::weak_ptr<rtc::VideoSinkInterface<webrtc::VideoFrame>> sink) {
