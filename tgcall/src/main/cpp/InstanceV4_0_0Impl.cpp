...
tgcalls::signaling_4_0_0::MediaContent BuildLocalMediaContent(
    ...
    const auto mapSsrc = [&ssrcMap](uint32_t remoteSsrc) {
        ...
        uint32_t localSsrc = rtc::CreateRandomNonZeroId();
        while (localSsrc == 0) {
            localSsrc = rtc::CreateRandomNonZeroId();
        }
    ...
    if (localContent.ssrc == 0) {
        localContent.ssrc = rtc::CreateRandomNonZeroId();
    }
...
bool RewriteReflectorCandidateAddressForWebK(
    ...
    for (const tgcalls::RtcServer &server : rtcServers) {
        ...
        if (!rtc::IPFromString(server.host, &ipAddress)) {
            return false;
        }
...
public:
    OutgoingAudioChannel_4_0_0(
        ...
        for (const auto &payloadType : mediaContent.payloadTypes) {
            ...
            cricket::AudioCodec codec = cricket::CreateAudioCodec(
                payloadType.id,
                payloadType.name,
            ...
            for (const auto &feedback : payloadType.feedbackTypes) {
                codec.AddFeedbackParam(cricket::FeedbackParam(feedback.type, feedback.subtype));
            }
            codecs.push_back(std::move(codec));
            break;
        }

        auto outgoingDescription = std::make_unique<cricket::AudioContentDescription>();
        ...
        auto incomingDescription = std::make_unique<cricket::AudioContentDescription>();
        for (const auto &rtpExtension : mediaContent.rtpExtensions) {
        ...
        _threads->getWorkerThread()->BlockingCall([this, &outgoingDescription, &incomingDescription]() {
            ...
            const bool localOk = _outgoingAudioChannel->SetLocalContent(outgoingDescription.get(), webrtc::SdpType::kOffer, errorDesc);
            const bool remoteOk = _outgoingAudioChannel->SetRemoteContent(incomingDescription.get(), webrtc::SdpType::kAnswer, errorDesc);
            OH_LOG_Print(LOG_APP, LOG_INFO, tgcall::kTgCallLogDomain, tgcall::kTgCallLogTag,
    ...
    ~OutgoingAudioChannel_4_0_0() {
        ...
        cricket::VoiceChannel *channelToDestroy = _outgoingAudioChannel;
    ...
    // M2.2 diagnostic: pulls send-side stats off the inner cricket::VoiceChannel without
    // exposing the raw pointer to callers. Must be invoked on the worker thread.
    bool getSendStats(cricket::VoiceMediaSendInfo *info) const {
        if (!_outgoingAudioChannel) {
            return false;
        }
        return _outgoingAudioChannel->send_channel()->GetStats(info);
    }
...
public:
    IncomingAudioChannel_4_0_0(
        ...
        incomingDescription->set_codecs(codecs);
        incomingDescription->set_bandwidth(-1);
        cricket::StreamParams streamParams = cricket::StreamParams::CreateLegacy(_ssrc);
        ...
        _threads->getWorkerThread()->BlockingCall([this, &outgoingDescription, &incomingDescription]() {
            ...
            std::string errorDesc;
            const bool localOk = _audioChannel->SetLocalContent(outgoingDescription.get(), webrtc::SdpType::kOffer, errorDesc);
            const bool remoteOk = _audioChannel->SetRemoteContent(incomingDescription.get(), webrtc::SdpType::kAnswer, errorDesc);
            OH_LOG_Print(LOG_APP, LOG_INFO, tgcall::kTgCallLogDomain, tgcall::kTgCallLogTag,
            ...
            _audioChannel->receive_channel()->SetOutputVolume(_ssrc, kIncomingVolumeBoost);
            OH_LOG_Print(LOG_APP, LOG_INFO, tgcall::kTgCallLogDomain, tgcall::kTgCallLogTag,
    ...
    ~IncomingAudioChannel_4_0_0() {
        ...
        cricket::VoiceChannel *channelToDestroy = _audioChannel;
        _threads->getNetworkThread()->BlockingCall([channelToDestroy]() {
            if (channelToDestroy) {
                channelToDestroy->SetRtpTransport(nullptr);
            }
...
private:
    ...
    void emitLocalCandidate(const cricket::Candidate &candidate);
    void emitMediaState();
    void onTransportMessageReceived(const rtc::CopyOnWriteBuffer &packet, bool isUnresolved);
    void onRtcpPacketReceived(const rtc::CopyOnWriteBuffer &packet, int64_t packetTimeUs);
    ...
    std::weak_ptr<rtc::VideoSinkInterface<webrtc::VideoFrame>> _incomingVideoOutput;
         ...
         enableP2P]() {
            return std::static_pointer_cast<tgcalls::InstanceNetworking>(
                std::make_shared<tgcalls::NativeNetworkingImpl>(tgcalls::InstanceNetworking::Configuration{
                    ...
                    .candidateGathered = [this](const cricket::Candidate &candidate) {
                        emitLocalCandidate(candidate);
                    },
                    .transportMessageReceived = [this](rtc::CopyOnWriteBuffer const &packet, bool isUnresolved) {
                        onTransportMessageReceived(packet, isUnresolved);
                    },
                    .rtcpPacketReceived = [this](rtc::CopyOnWriteBuffer const &packet, int64_t packetTimeUs) {
...
InstanceV4_0_0ImplInternal::createAudioDeviceModule() {
    ...
    return check(webrtc::AudioDeviceModule::Create(
...
void InstanceV4_0_0ImplInternal::setUpMediaPipeline() {
    ...
    mediaDependencies.task_queue_factory = _taskQueueFactory.get();
    ...
    mediaDependencies.audio_processing = _audioProcessing;
    ...
    // Video factories intentionally nullptr — M1 audio-only.
    mediaDependencies.trials = &fieldTrialsBasedConfig;

    auto mediaEngine = cricket::CreateMediaEngine(std::move(mediaDependencies));
    if (!mediaEngine) {
...
void InstanceV4_0_0ImplInternal::createNegotiatedChannels() {
    ...
    for (const auto &content : coordinatedState->incomingContents) {
        ...
        if (_incomingAudioChannel && _incomingAudioChannel->ssrc() != content.ssrc) {
            ...
            // cricket::ChannelManager docs. Resetting from the signaling thread crashed the
    ...
    // cricket::ChannelManager docs ("operations all occur on the worker thread"). The crash
    ...
    if (!_outgoingAudioChannel && _outgoingAudioOffer.has_value()) {
        ...
        if (_outgoingAudioChannel && !_outgoingAudioChannel->isReady()) {
            ...
            const auto logSendStats = [this, alive, ssrcSnapshot](int tSeconds) {
                ...
                cricket::VoiceMediaSendInfo sendInfo;
                ...
                if (sendOk) {
                    for (const auto &sender : sendInfo.senders) {
                        if (sender.ssrc() == ssrcSnapshot || sender.local_stats.empty()) {
                            ...
                            rttMs = sender.rtt_ms;
                            targetBitrate = sender.target_bitrate.value_or(0.0);
                            matched = true;
                            break;
                        }
                    }
...
void InstanceV4_0_0ImplInternal::teardownMediaPipelineOnWorkerThread() {
    ...
    // cricket::ChannelManager docs: "operations all occur on the worker thread".
...
void InstanceV4_0_0ImplInternal::configureRemoteInitialSetup(
    ...
    _networking->perform([remoteInitialSetup, isCallee](tgcalls::InstanceNetworking *networking) {
        ...
        std::unique_ptr<rtc::SSLFingerprint> remoteFingerprint;
        std::string sslSetup;
        if (!remoteInitialSetup.fingerprints.empty()) {
            ...
            remoteFingerprint = rtc::SSLFingerprint::CreateUniqueFromRfc4572(
                fingerprint.hash,
...
void InstanceV4_0_0ImplInternal::addRemoteCandidates(
    ...
    _networking->perform([remoteCandidates](tgcalls::InstanceNetworking *networking) {
        std::vector<cricket::Candidate> parsedCandidates;
        ...
        for (const tgcalls::signaling_4_0_0::IceCandidate &candidate : remoteCandidates.iceCandidates) {
            webrtc::JsepIceCandidate parsedCandidate(std::string(), 0);
            if (!parsedCandidate.Initialize(candidate.sdpString, nullptr)) {
...
void InstanceV4_0_0ImplInternal::emitLocalInitialSetup(
    ...
    _networking->perform([this, remoteInitialSetup, answerCopy = std::move(answerCopy)](tgcalls::InstanceNetworking *networking) {
        ...
        std::unique_ptr<rtc::SSLFingerprint> localFingerprint = networking->getLocalFingerprint();
        if (!localFingerprint) {
...
void InstanceV4_0_0ImplInternal::emitLocalCandidate(const cricket::Candidate &candidate) {
    ...
    if (patchedCandidate.protocol() != cricket::UDP_PROTOCOL_NAME) {
    ...
    rtc::SocketAddress candidateAddress = patchedCandidate.address();
    ...
    rtc::SocketAddress relatedCandidateAddress = patchedCandidate.related_address();
...
void InstanceV4_0_0Impl::setIncomingVideoOutput(std::weak_ptr<rtc::VideoSinkInterface<webrtc::VideoFrame>> sink) {
