...
bool IsReflectorCandidateAddress(const rtc::SocketAddress &address) {
  ...
  networking_->perform([remoteCandidates](tgcalls::InstanceNetworking *networking) {
    ...
    for (const tgcalls::signaling_4_0_0::IceCandidate &candidate : remoteCandidates.iceCandidates) {
      ...
      OH_LOG_Print(LOG_APP, LOG_INFO, kTgCallLogDomain, kTgCallLogTag,
                   ...
                   parsedCandidate.candidate().protocol().c_str(),
                   parsedCandidate.candidate().type().c_str(),
                   parsedCandidate.candidate().relay_protocol().c_str(),
                   parsedCandidate.candidate().address().ToString().c_str(),
                   parsedCandidate.candidate().related_address().ToString().c_str(),
                   candidate.sdpString.size());
...
void DiagnosticTgCallInstance::emitLocalCandidate(const cricket::Candidate &candidate) {
...
void DiagnosticTgCallInstance::onTransportMessageReceived(const rtc::CopyOnWriteBuffer &packet, bool isUnresolved) {
...
void DiagnosticTgCallInstance::onRtcpPacketReceived(const rtc::CopyOnWriteBuffer &packet, int64_t packetTimeUs) {
  size_t packetCount = 0;
  size_t byteCount = 0;
  {
    std::lock_guard<std::mutex> lock(mediaDiagnosticsMutex_);
